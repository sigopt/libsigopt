...
class MultitaskTensorCovariance(DifferentiableCovariance):
  ...
  def translation_invariant(self):
    return (
      self.physical_covariance.translation_invariant
      and self.task_covariance.translation_invariant
    )
  ...
  def _build_kernel_matrix(self, points_sampled, points_to_sample=None):
    x_phys, x_task, z_phys, z_task = self.separate_physical_task_components(
  ...
  def _grad_covariance(self, x, z):
    ...
    physical_covariance_grad_tensor = self.physical_covariance._grad_covariance(
      x_phys, z_phys
    )
    ...
    grad_covariance_tensor[:, :-1] = (
      physical_covariance_grad_tensor * task_covariance_vector[:, None]
    )
    grad_covariance_tensor[:, -1:] = (
      task_covariance_grad_tensor * physical_covariance_vector[:, None]
    )
  ...
  def _hyperparameter_grad_covariance_without_process_variance(self, x, z):
    ...
    task_covariance_vector = self.task_covariance._covariance(x_task, z_task)
    phys_hgc = (
      self.physical_covariance._hyperparameter_grad_covariance_without_process_variance(
        x_phys, z_phys
      )
    )
    task_hgc = (
      self.task_covariance._hyperparameter_grad_covariance_without_process_variance(
        x_task, z_task
      )
    )
    ...
    hparam_grad_covariance_tensor[:, -1:] = (
      task_hgc * physical_covariance_vector[:, None]
    )
  ...
  def _build_kernel_hparam_grad_tensor_without_process_variance(
    ...
    phys_matrix = self.physical_covariance._build_kernel_matrix(x_phys, z_phys)
    task_matrix = self.task_covariance._build_kernel_matrix(x_task, z_task)
    phys_kgt = self.physical_covariance._build_kernel_hparam_grad_tensor_without_process_variance(
      x_phys, z_phys
    )
    task_kgt = (
      self.task_covariance._build_kernel_hparam_grad_tensor_without_process_variance(
        x_task, z_task
      )
    )
