...
from test.compute.zigopt_input_utils import (
  form_points_sampled,
  form_random_hyperparameter_dict,
)


def fill_random_covariance_hyperparameters(
  interval, num_hyperparameters, covariance_type
):
  assert len(interval) == 2
  return covariance_type(
    [numpy.random.uniform(*interval) for _ in range(num_hyperparameters)]
  )
...
class GaussianProcessTestCase(NumericalTestCase):
  ...
  def domain_list(self):
    dim_list = [1, 3, 5, 9]
    return [
      self.form_continous_and_uniform_domain(
        dim=dim, lower_element=-1, higher_element=1
      )
      for dim in dim_list
    ]
  ...
  def any_gaussian_process_list(self, any_domain_list):
    return [
      self.form_gaussian_process_and_data(domain, num_sampled)
  ...
  def list_probabilistic_failures_list(self, request, domain_list):
    ...
    return [
      self.form_list_of_probabilistic_failures(
        num_gp, domain, num_sampled, pf_cdf=request.param
      )
      for domain, num_sampled, num_gp in zip(
        domain_list, num_gps_list, num_sampled_list
      )
    ]

  @pytest.fixture(scope=GP_FIXTURE_SCOPE)
  def product_of_list_probabilistic_failures_list(
    self, list_probabilistic_failures_list
  ):
    return [
      ProductOfListOfProbabilisticFailures(list_of_pfs)
      for list_of_pfs in list_probabilistic_failures_list
    ]
  ...
  @staticmethod
  def form_list_of_probabilistic_failures(
    num_gps, domain, num_sampled, noise_per_point=1e-2, pf_cdf=True
  ):
    list_of_pfs = []
  ...
  @staticmethod
  def form_gaussian_process_and_data(
    domain, mpi=None, num_sampled=50, noise_per_point=0.002
  ):
    if mpi is None:
  ...
  def form_gaussian_process(
    ...
    noise_variance = (
      numpy.zeros(num_points) if noise_variance is None else noise_variance
    )
    ...
    historical_data.append_historical_data(
      points_sampled, scaled_values, scaled_variance
    )

    mean_poly_indices = [[]] if mean_poly_indices is None else mean_poly_indices
    return GaussianProcess(
      covariance, historical_data, mean_poly_indices, tikhonov_param=tikhonov_param
    )
