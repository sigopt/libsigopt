...
class NumericalTestCase(object):
  ...
  def assert_scalar_within_relative(value, truth, tol):
    ...
    assert (
      diff <= tol
    ), f"value = {value:.18E}, truth = {truth:.18E}, diff = {diff:.18E}, tol = {tol:.18E}"
  ...
  def assert_vector_within_relative(value, truth, tol):
    ...
    ), f"value.shape = {value.shape} != truth.shape = {truth.shape}"
  ...
  def assert_vector_row_wise_norm_is_close(
    value, truth, tol, norm=2, abs_tol=DEFAULT_ABS_TOL
  ):
    assert (
      value.shape == truth.shape
    ), f"value.shape = {value.shape} != truth.shape = {truth.shape}"
  ...
  def assert_vector_within_relative_norm(value, truth, tol, norm=2):
    ...
    mag = (
      numpy.linalg.norm(t, ord=norm)
      if numpy.linalg.norm(t, ord=norm) > numpy.finfo(numpy.float64).eps
      else 1.0
    )
  ...
  def check_gradient_with_finite_difference(
    x, func, grad, fd_step, tol, use_complex=False
  ):
    """
    ...
    def fd_centered_method(x, func, fd_step, g_approx):
      ...
      for i in range(dim):
        g_approx[:, i] = (
          func(x_plus_perturbation[:, :, i]) - func(x_min_perturbation[:, :, i])
        ) / (2 * fd_step[i])
      return g_approx
